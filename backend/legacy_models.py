"""
Legacy in-memory data store for backward compatibility.
This is a temporary solution to keep existing RAG functionality working
while the migration to the new database system is completed.
"""
...
class InMemoryStore:
    ...
    def delete_assessment(self, session_id: str, assessment_id: int) -> bool:
        s = self.get_session(session_id)
        if assessment_id not in s.assessments:
            return False
        del s.assessments[assessment_id]
        # Delete related results
        result_ids_to_delete = [r.id for r in s.assessment_results.values() if r.assessment_id == assessment_id]
